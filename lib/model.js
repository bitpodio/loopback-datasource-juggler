...
function ModelBaseClass(data, options) {
  ...
  try {
    ...
    console.error('Failed to intialize model properties ', error);
    console.error(data);
  }
}
...
ModelBaseClass.prototype._initProperties = function(data, options) {
  ...
  for (k = 0; k < size; k++) {
    ...
    if (!BASE_TYPES[type.name]) {
      ...
      } else if (type.name === 'GeoPoint') {
        console.log('Type GeoPoint');
        console.log('Data Object: ', JSON.stringify(self.__data, null, 2));
        console.log('Filed name : ', p);
        if (self.__data[p] && self.__data[p].coordinates && self.__data[p].coordinates.length === 2) {
          self.__data[p] = {
            lat: self.__data[p].coordinates[1],
            lng: self.__data[p].coordinates[0],
          };
        }
      }
    }

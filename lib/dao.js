...
function isWhereByGivenId(Model, where, idValue) {
  ...
  var pk = idName(Model);
  if (keys[0] !== pk && !where['and'][0][pk]) return false;

  return where[pk] === idValue || where['and'][0][pk] === idValue;
  //Fix to allow user given id in put and patch api when soft delete is applied
}
      ...
      Model.notifyObserversOf('before save', context, function(err, ctx) {
        ...
        function callConnector() {
          ...
          Model.notifyObserversOf('persist', context, function(err) {
            ...
            if (connector.updateOrCreate.length === 5) {
              var filter = {};
              if (Model.base.modelName === 'TenantizedModel') {
                filter = {where: ctx.where};
              }
              connector.updateOrCreate(Model.modelName, filter, update, options, done);
            } else if (connector.updateOrCreate.length === 4) {
      ...
      if (val != null) {
        if (operator === null && val instanceof RegExp) {
          ...
          // Do not coerce regex literals/objects
            ...
            operator === 'ilike' || operator === 'nilike') && val instanceof RegExp) && !props[p].isArray  && !(val instanceof Object)) {
          val = DataType(val);
        }
      }
    }
